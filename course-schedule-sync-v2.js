...
function hasFixedDates(schedule){
  const dates=schedule?.dates;
  return Boolean(dates&&typeof dates==="object"&&Object.keys(dates).some(key=>FIXED_KEYS.has(key)&&dates[key]));
}
...
function scheduleDraft(row,preferredStudentId=""){
  draftSourceName="";
  if(row.schedule&&typeof row.schedule==="object")return row.schedule;
  const members=courseMembers(row.course_key);
  const preferred=members.find(student=>String(student.id)===String(preferredStudentId));
  const candidates=[preferred,...members].filter(Boolean);
  const seen=new Set();
  for(const student of candidates){
    if(seen.has(String(student.id)))continue;
    seen.add(String(student.id));
    const schedule=parseScheduleFromNotes(student.notes||"");
    if(hasFixedDates(schedule)){
      draftSourceName=student.name||"một học viên trong khóa";
      return schedule;
    }
  }
  return{dates:{},locations:{},note:""};
}
...
function prepareCourse(row,preferredStudentId=""){
  if(!row)return;
  const members=courseMembers(row.course_key);
  const select=$("scheduleCourse");
  if(select)select.value=row.course_key;
  setRepresentative(members);
  renderFields(row,preferredStudentId);
...
function installUi(){
  if(mounted)return;
  const form=$("scheduleForm"),dialog=$("scheduleDialog"),originalLabel=form?.querySelector(".student-select"),originalSelect=$("scheduleStudent");
  if(!form||!dialog||!originalLabel||!originalSelect)return;
  mounted=true;
  dialog.classList.add("course-schedule-dialog");
  ...
  document.addEventListener("click",event=>{
    ...
    if(remove&&FIXED_KEYS.has(remove.dataset.deleteKey)){
      event.preventDefault();event.stopImmediatePropagation();
