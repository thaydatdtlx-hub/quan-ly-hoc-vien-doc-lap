...
function mountIntakes(){
  ...
  const node=section("site-intakes","lich-khai-giang",`${heading("LỊCH TIẾP NHẬN HỒ SƠ","Đăng ký trước để được xếp khóa phù hợp","Ngày khai giảng và số chỗ còn lại cần được xác nhận theo từng hạng; website không hiển thị số lượng giả định.")}<div class="site-intakes-grid">${[["A1","A1","Lịch tiếp nhận hồ sơ được xác nhận khi tư vấn."],["A","A","Lịch tiếp nhận hồ sơ được xác nhận khi tư vấn."],["B số tự động","B số tự động","Thời gian đào tạo dự kiến 2,5–3 tháng."],["B số sàn","B số sàn","Thời gian đào tạo dự kiến 2,5–3 tháng."],["C1","C1","Thời gian đào tạo dự kiến 3,5–4 tháng."]].map(([tag,license,text])=>`<article class="site-intake-card"><span>ĐANG NHẬN TƯ VẤN</span><h3>${tag}</h3><p>${text}</p><ul><li>Ngày bắt đầu: xác nhận khi tư vấn</li><li>Hạn hồ sơ: theo kế hoạch khóa</li><li>Số chỗ: cập nhật trực tiếp</li></ul><button type="button" data-intake-license="${license}">Đăng ký giữ thông tin</button></article>`).join("")}</div>`);
