...
function warning(message="",retry=false){
  ...
  if(!message){document.querySelectorAll("#studentRuntimeWarning").forEach(node=>node.remove());return}
...
function finishProfileSync(){
  ...
  warning("");showPortal();document.documentElement?.setAttribute("data-student-profile","ready");
  setText("mobileStudentActionTitle","Hồ sơ đã sẵn sàng");
  setText("mobileStudentActionDetail","Lịch học và các dữ liệu mới nhất đã sẵn sàng.");
  return true;
}
...
function renderCore(){
  ...
  finishProfileSync();window.dispatchEvent(new CustomEvent("student-profile-ready"));void renderQr(debt);return true;
}
function wire(){
...
async function loadUsername(){try{const me=await rpcReliable("app_student_me",{p_token:token});if(me?.username)setText("studentUsername",`${me.username} · Học viên`)}catch{}finally{finishProfileSync()}}
async function boot(force=false){
  ...
  try{
    ...
    student=result;if(!renderCore())throw new Error("Không thể hiển thị hồ sơ học viên.");
    for(const delay of [120,800,2400])setTimeout(finishProfileSync,delay);
    void loadUsername();
  }catch(error){
...
window.addEventListener("pageshow",finishProfileSync);
document.addEventListener("visibilitychange",()=>{if(!document.hidden)finishProfileSync()});
boot();
