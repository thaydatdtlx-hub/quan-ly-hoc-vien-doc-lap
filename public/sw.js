...
const CACHE_NAME="thay-dat-pwa-v36";
const CORE_ASSETS=[
  "/",
  "/index.html",
  "/dang-nhap.html",
  ...
  "/student-safe-runtime-v2.js?v=2",
...
self.addEventListener("fetch",event=>{
  ...
  const isStudent=url.pathname==="/hoc-vien.html"||url.pathname.includes("student-safe-runtime");
  if(request.mode==="navigate"){
    event.respondWith(fetch(request,isStudent?{cache:"no-store"}:undefined).then(response=>{if(response.ok){const copy=response.clone();caches.open(CACHE_NAME).then(cache=>cache.put(request,copy))}return response}).catch(async()=>await caches.match(request,{ignoreSearch:true})||await caches.match(url.pathname)||await caches.match("/offline.html")));return;
  }
  if(request.destination==="script"||request.destination==="style"){
    event.respondWith(fetch(request,isStudent?{cache:"no-store"}:undefined).then(response=>{if(response.ok&&response.type==="basic"){const copy=response.clone();caches.open(CACHE_NAME).then(cache=>cache.put(request,copy))}return response}).catch(()=>caches.match(request)));return;
  }
  event.respondWith(caches.match(request).then(cached=>{const network=fetch(request).then(response=>{if(response.ok&&response.type==="basic"){const copy=response.clone();caches.open(CACHE_NAME).then(cache=>cache.put(request,copy))}return response}).catch(()=>cached);return cached||network}));
});
