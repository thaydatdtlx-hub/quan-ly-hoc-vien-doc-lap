...
const CACHE_NAME="thay-dat-pwa-v41";
const CORE_ASSETS=["/?login=1","/dang-ky-hoc-lai-xe.html","/lich-dao-tao.html","/600-cau-hoi.html","/bo-tuc-tay-lai.html","/chinh-sach-bao-mat.html","/offline.html","/site.webmanifest","/mobile-viewport-lock.css?v=3","/app-icon-192.png","/app-icon-512.png","/app-icon-maskable-512.png","/apple-touch-icon-180.png","/logo-thay-dat-compact.webp"];
...
self.addEventListener("fetch",event=>{
  ...
  if(request.destination==="script"||request.destination==="style"){
    event.respondWith(fetch(request).then(response=>{if(response.ok&&response.type==="basic"){const copy=response.clone();caches.open(CACHE_NAME).then(cache=>cache.put(request,copy))}return response}).catch(()=>caches.match(request)));return;
  }
